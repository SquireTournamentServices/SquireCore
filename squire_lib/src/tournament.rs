...
impl TournamentSeed {
    ...
    pub fn default_name() -> String {
        let mut bytes = [0; 8];
        let _ = getrandom::getrandom(&mut bytes); // should squire_lib be the source of randomness?
        let random = u64::from_ne_bytes(bytes);
        let random_upper = (random >> 32) as usize;
        let random_lower = ((random << 32) >> 32) as usize;

        fn select(slice: &[&'static str], n: usize) -> &'static str {
            slice[n % slice.len()]
        }

        format!(
            "{}: the {}",
            select(Self::MAGIC_SYNONYMS, random_upper),
            select(Self::GATHERING_SYNONYMS, random_lower)
        )
    }
